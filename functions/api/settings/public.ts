...
export const onRequest = async (context: any) => {
    ...
    try {
        ...
        return new Response(JSON.stringify({
            ...
            iphone_pwa_guide_bindings: (() => {
                try {
                    return settings['iphone_pwa_guide_bindings'] ? JSON.parse(settings['iphone_pwa_guide_bindings']) : {};
                } catch {
                    return {};
                }
            })(),
            // Native 안드로이드 앱 업데이트 관리 설정
            android_update_enabled: settings['android_update_enabled'] === 'true' || settings['android_update_enabled'] === '1',
            android_update_type: settings['android_update_type'] === 'mandatory' ? 'mandatory' : 'recommended',
            android_latest_version_code: parseInt(settings['android_latest_version_code'] || '5', 10),
            android_latest_version_name: settings['android_latest_version_name'] || '1.0.4',
            android_min_version_code: parseInt(settings['android_min_version_code'] || '3', 10),
            android_update_title: settings['android_update_title'] || '새로운 버전 업데이트 안내',
            android_update_notes: settings['android_update_notes'] || '• 푸시/배지/헤드업 배너 알림 수신 기능 개선\n• 앱 접속 안정성 향상',
