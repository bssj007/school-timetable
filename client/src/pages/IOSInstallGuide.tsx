      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            {/* 하단 액션 바 — 더 보기에 강조 원 + 돼지꼬리 화살표 */}
            <div className="relative" style={{ paddingTop: '44px', overflow: 'visible' }}>
              {/* ── 강조 원 + 돼지꼬리 화살표 SVG 오버레이 ── */}
              {/* SVG는 하단 바 오른쪽 25% 영역(더 보기 셀)을 덮도록 절대 배치 */}
              <svg
                className="absolute pointer-events-none"
                style={{ top: 0, right: 0, width: '30%', height: '100%', overflow: 'visible' }}
                viewBox="0 0 80 120"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                {/* 강조 원 — 더 보기 버튼을 감싸는 타원 */}
                <ellipse cx="40" cy="100" rx="37" ry="21" stroke="#ef4444" strokeWidth="2.5" />
                {/* 돼지꼬리 화살표 — 원 상단에서 시작해서 말리는 곡선 */}
                <path
                  d="M 42 79 C 42 58, 72 50, 70 64 C 68 76, 50 70, 55 58"
                  stroke="#ef4444"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                  fill="none"
                />
                {/* 화살촉 */}
                <path d="M 55 58 L 49 55" stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" />
                <path d="M 55 58 L 58 52" stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" />
              </svg>
              ...
              <div className="flex items-center gap-0 divide-x divide-gray-100 bg-white">
                {[
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center gap-0.5 py-2">
                    {action.icon}
                    <span className="text-[9px] text-gray-400">{action.label}</span>
                  </div>
                ))}
                {/* 더 보기 — 기본 스타일 그대로 (강조는 SVG 오버레이로만) */}
